...
import com.yourssu.scouter.recruiting.recruiter.business.CreateScheduleCommand
import com.yourssu.scouter.recruiting.recruiter.business.ScheduleService
...
class ScheduleController(
    ...
    fun createSchedules(
        @RequestBody schedules: List<CreateScheduleRequest>,
    ): ResponseEntity<Unit> {
    ...
    fun getAutoSchedules(
        @PathVariable partId: Long,
        request: AutoScheduleRequest,
    ) = ResponseEntity.ok(
        scheduleService.autoGenerateSchedules(partId, request.strategy, request.duration, request.size).map {
    ...
    fun deleteByPart(
        @PathVariable partId: Long,
    ): ResponseEntity<DeleteByPartResponse> {
    ...
    @PatchMapping("/schedule/{scheduleId}/location")
    fun updateLocation(
        @PathVariable scheduleId: Long,
        @RequestBody @Valid request: UpdateScheduleLocationRequest,
    ): ResponseEntity<Unit> {
        scheduleService.updateLocation(request.toCommand(scheduleId))
        return ResponseEntity.ok().build()
    }
}
