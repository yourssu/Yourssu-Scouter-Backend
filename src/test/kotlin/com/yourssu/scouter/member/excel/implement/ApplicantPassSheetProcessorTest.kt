...
class ApplicantPassSheetProcessorTest {
    ...
    fun setUp() {
        ...
        whenever(memberPartRoleResolver.toPartAndRoles(any(), any(), any()))
            .thenReturn(MemberPartAndRoles(setOf(MemberPartAndRole(part, com.yourssu.scouter.member.core.implement.MemberRole.MEMBER))))
    ...
    inner class ParseSingleRow {
        ...
        fun `유효한 한 행이면 멤버가 ACTIVE로 저장되고 에러 없음`() {
            ...
            val captor = argumentCaptor<com.yourssu.scouter.member.core.implement.Member>()
            ...
            assertThat(member.state).isEqualTo(com.yourssu.scouter.member.core.implement.MemberState.ACTIVE)
    ...
    inner class ParseDuplicateInSheet {
        ...
        fun `같은 전화번호가 두 행이어도 에러 없이 첫 행은 신규 저장 후 둘째 행은 해당 멤버 업데이트`() {
            ...
            whenever(memberReader.readActiveByMemberId(existingMember.id!!)).thenReturn(
                com.yourssu.scouter.member.core.implement.ActiveMember(
                    id = 1L,
            ...
            verify(memberWriter, org.mockito.kotlin.times(1)).update(any<com.yourssu.scouter.member.core.implement.ActiveMember>())
        }
    }
    ...
    inner class ParseDuplicateInDb {
        ...
        fun `전화번호가 이미 DB에 있으면 해당 멤버를 시트 값으로 패치`() {
            ...
            assertThat(result.hasErrors()).isFalse()
            verify(memberWriter, org.mockito.kotlin.never()).writeMemberWithActiveStatus(any(), any(), any(), any())
            val captor = argumentCaptor<com.yourssu.scouter.member.core.implement.ActiveMember>()
        ...
        fun `학번이 이미 DB에 있으면 해당 멤버를 시트 값으로 패치`() {
            ...
            assertThat(result.hasErrors()).isFalse()
            verify(memberWriter).update(any<com.yourssu.scouter.member.core.implement.ActiveMember>())
        }
    }
