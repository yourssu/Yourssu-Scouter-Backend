    ...
    inner class ParseNormalRow {
        ...
        fun `이름+부서로 단일 멤버를 찾아 WITHDRAWN 상태로 전환하고 시트 비고를 note에 반영한다`() {
            ...
            whenever(memberReader.searchAllActiveByNameOrNickname("홍길동"))
                .thenReturn(listOf(com.yourssu.scouter.hrms.member.implement.ActiveMember(id = 1L, member = existingMember, isMembershipFeePaid = false)))
    ...
    inner class ParseSearchFailures {
        ...
        fun `이름+부서에 해당하는 멤버가 여러 명이면 에러 메시지에 닉네임이 포함된다`() {
            ...
            whenever(memberReader.searchAllActiveByNameOrNickname("중복이름"))
                .thenReturn(
                    listOf(
                        com.yourssu.scouter.hrms.member.implement.ActiveMember(id = 1L, member = member1, isMembershipFeePaid = false),
                        com.yourssu.scouter.hrms.member.implement.ActiveMember(id = 2L, member = member2, isMembershipFeePaid = false),
                    ),
                )
